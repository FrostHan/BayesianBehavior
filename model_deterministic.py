...
class DeterministicAgent(nn.Module):
    def __init__(self,
        ...
        self.x_phi_layers = vrnn_config["x_phi_layers"] if ("x_phi_layers" in vrnn_config) else [hidden_size, self.z_size]
    ...
    def compute_z(self, x_t_obs):
        
        if x_t_obs.shape.__len__() >= 5:
            z_q_t = self.f_x2phi_z(x_t_obs.reshape(-1, x_t_obs.shape[-3:])).reshape([x_t_obs.shape[0], x_t_obs.shape[1], -1])
        else:
            z_q_t = self.f_x2phi_z(x_t_obs)

        return z_q_t, z_q_t, torch.zeros_like(z_q_t)
    ...
    def step_with_env(self, env, x_prev, action_return='normal', action_filter=None, prior_z_funtion=None):
        ...
        with torch.no_grad():
            ...
            self.z_p, self.mu_z_p, self.sig_z_p = self.compute_z(x_prev_tensor)
            ...
            self.z_q, self.mu_z_q, self.sig_z_q = self.compute_z(x_prev_tensor)
    ...
    def learn(self, buffer, minibatch_size):
        ...
        for stp in range(max_stps + 1):
            z_q_t, muz_q_t, sigz_q_t = self.compute_z(x_tensor[:, stp])
